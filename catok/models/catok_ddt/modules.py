...
from catok.models.catok_ddt.models import DiT, DiTBlock, get_2d_sincos_pos_embed, modulate, TimestepEmbedder, FinalLayer
import torch.nn.functional as F
from torch.nn import init
from catok.models.catok_ddt.sd3.sd3_utils import is_torch_npu_available
...
class DualBlock(ViTBlock):
    ...
    def __init__(self, hidden_size, num_heads, mlp_ratio=4.0, time_adaln=False, query_dim=256, diti=None, **block_kwargs):
        ...
        # Filter out parameters that DualAttention doesn't accept
        attn_kwargs = {k: v for k, v in block_kwargs.items() 
                      if k not in ['layernorm_elementwise_affine', 'layernorm_bias', 'linear_bias', 'embedder_type']}
        super().__init__(hidden_size, num_heads, mlp_ratio, attn=DualAttention, **attn_kwargs)
        q_dim = query_dim
        # Explicitly set q norm scalable
        self.q_norm1 = nn.LayerNorm(q_dim, elementwise_affine=True, eps=1e-6) if not self.post_ln else nn.Identity()
        self.q_norm2 = nn.LayerNorm(q_dim, elementwise_affine=True, eps=1e-6) if not self.post_ln else nn.Identity()
        self.post_norm1 = nn.LayerNorm(q_dim, elementwise_affine=True, eps=1e-6) if self.post_ln else nn.Identity()
        self.post_norm2 = nn.LayerNorm(q_dim, elementwise_affine=True, eps=1e-6) if self.post_ln else nn.Identity()
...
class DualBlockMultiRes(ViTBlock):
    ...
    def __init__(self, hidden_size, num_heads, mlp_ratio=4.0, time_adaln=False, query_dim=512, diti=None, low_res_hidden_dim=64, **block_kwargs):
        block_kwargs["query_dim"] = query_dim
        self.post_ln = block_kwargs.pop("post_ln", False)
        # Filter out parameters that DualAttentionMultiRes doesn't accept
        attn_kwargs = {k: v for k, v in block_kwargs.items() 
                      if k not in ['layernorm_elementwise_affine', 'layernorm_bias', 'linear_bias', 'embedder_type']}
        super().__init__(hidden_size, num_heads, mlp_ratio, attn=DualAttentionMultiRes, **attn_kwargs)
        q_dim = query_dim
