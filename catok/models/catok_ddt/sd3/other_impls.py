...
def attention(q, k, v, heads, mask=None):
    ...
    # from catok.utils.visualize import visualize_attn_mask
    # for i in range(5):
    #      visualize_attn_mask(mask[i][0], "tmp", file_name=f"attn_mask_{i}.png")
...
class CLIPLayer(torch.nn.Module):
    def __init__(self, embed_dim, heads, intermediate_size, intermediate_activation, dtype, device, layernorm_elementwise_affine=True):
        super().__init__()
        self.layer_norm1 = nn.LayerNorm(embed_dim, elementwise_affine=layernorm_elementwise_affine, dtype=dtype, device=device)
        self.self_attn = CLIPAttention(embed_dim, heads, dtype, device)
        self.layer_norm2 = nn.LayerNorm(embed_dim, elementwise_affine=layernorm_elementwise_affine, dtype=dtype, device=device)
...
class CLIPEncoder(torch.nn.Module):
    def __init__(self, num_layers, embed_dim, heads, intermediate_size, intermediate_activation, dtype, device, layernorm_elementwise_affine=True):
        super().__init__()
        self.layers = torch.nn.ModuleList(
            [
                CLIPLayer(embed_dim, heads, intermediate_size, intermediate_activation, dtype, device, layernorm_elementwise_affine=layernorm_elementwise_affine)
...
class CLIPTextModel_(torch.nn.Module):
    def __init__(self, config_dict, dtype, device, layernorm_elementwise_affine=True):
        ...
        self.encoder = CLIPEncoder(
            num_layers, embed_dim, heads, intermediate_size, intermediate_activation, dtype, device, layernorm_elementwise_affine=layernorm_elementwise_affine
        )
        self.final_layer_norm = nn.LayerNorm(embed_dim, elementwise_affine=layernorm_elementwise_affine, dtype=dtype, device=device)
