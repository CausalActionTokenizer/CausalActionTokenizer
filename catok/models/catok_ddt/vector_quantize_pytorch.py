...
def calc_ema_entropy(distances, onehot_distances, ratio_d=0.3):
    ...
    groups = 64 if ema_p.shape[0] >= 64 else ema_p.shape[0]
    ema_p_group = torch.stack([t.mean(dim=0) for t in ema_p.tensor_split(groups, dim=0)],dim=0)
...
class CosineSimCodebook(nn.Module):
    ...
    def forward(
        ...
        if self.training and not self.dead_code_threshold_updated:
            ...
            ratio = x.shape[0] * x.shape[1] * world_size / self.codebook_size
...
class VectorQuantize(nn.Module):
    def __init__(
        ...
        # Calculate effective learnable_codebook value that will be passed to CosineSimCodebook
        effective_learnable_codebook = has_codebook_orthogonal_loss or learnable_codebook
        assert not (ema_update and effective_learnable_codebook), 'learnable codebook (including orthogonal_reg_weight > 0) not compatible with EMA update'
